...
import (
	...
	"github.com/sail-services/sail-go/foundation/framework/service/session"
	"strings"
	"sync"
	"time"
...
func (s *RedisStore) Release() error {
	...
	return s.c.SetEx(s.prefix, s.duration, string(data)).Err()
}
...
func (p *RedisProvider) Init(maxlifetime int64, configs string) (err error) {
	...
	for k, v := range cfg.Section("").KeysHash() {
		...
		case "password":
			opt.Password = v
		case "db":
			opt.DB = convert.STo(v).MustI64()
		case "pool_size":
			opt.PoolSize = convert.STo(v).MustI()
		case "idle_timeout":
