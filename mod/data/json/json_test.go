...
func TestSimplejson(t *testing.T) {
	...
	var awsval int
	awsval, _ = aws.GetIndex(0).Get("subkeyone").I()
	assert.Equal(t, 1, awsval)
	awsval, _ = aws.GetIndex(1).Get("subkeytwo").I()
	assert.Equal(t, 2, awsval)
	awsval, _ = aws.GetIndex(1).Get("subkeythree").I()
	assert.Equal(t, 3, awsval)

	i, _ := js.Get("test").Get("int").I()
	assert.Equal(t, 10, i)

	f, _ := js.Get("test").Get("float").F64()
	assert.Equal(t, 5.150, f)

	s, _ := js.Get("test").Get("string").S()
	assert.Equal(t, "simplejson", s)

	b, _ := js.Get("test").Get("bool").Bool()
	assert.Equal(t, true, b)

	mi := js.Get("test").Get("int").MustI()
	assert.Equal(t, 10, mi)

	mi2 := js.Get("test").Get("missing_int").MustI(5150)
	assert.Equal(t, 5150, mi2)

	ms := js.Get("test").Get("string").MustS()
	assert.Equal(t, "simplejson", ms)

	ms2 := js.Get("test").Get("missing_string").MustS("fyea")
	...
	strs, err := js.Get("test").Get("string_array").SArray()
	assert.Equal(t, err, nil)
	assert.Equal(t, strs[0], "asdf")
	assert.Equal(t, strs[1], "ghjk")
	assert.Equal(t, strs[2], "zxcv")

	strs2, err := js.Get("test").Get("string_array_null").SArray()
	assert.Equal(t, err, nil)
	assert.Equal(t, strs2[0], "abc")
	assert.Equal(t, strs2[1], "")
	assert.Equal(t, strs2[2], "efg")

	gp, _ := js.GetPath("test", "string").S()
	assert.Equal(t, "simplejson", gp)

	gp2, _ := js.GetPath("test", "int").I()
	assert.Equal(t, 10, gp2)

	assert.Equal(t, js.Get("test").Get("bool").MustBool(), true)

	js.Set("float2", 300.0)
	assert.Equal(t, js.Get("float2").MustF64(), 300.0)

	js.Set("test2", "setTest")
	assert.Equal(t, "setTest", js.Get("test2").MustS())

	js.Del("test2")
	assert.NotEqual(t, "setTest", js.Get("test2").MustS())

	js.Get("test").Get("sub_obj").Set("a", 2)
	assert.Equal(t, 2, js.Get("test").Get("sub_obj").Get("a").MustI())

	js.GetPath("test", "sub_obj").Set("a", 3)
	assert.Equal(t, 3, js.GetPath("test", "sub_obj", "a").MustI())
}
...
func TestSet_Path(t *testing.T) {
	...
	s, err := js.GetPath("foo", "bar").S()
	assert.Equal(t, nil, err)
	assert.Equal(t, "baz", s)
}

func TestSet_PathNoPath(t *testing.T) {
	js, err := NewB([]byte(`{"some":"data","some_number":1.0,"some_bool":false}`))
	assert.Equal(t, nil, err)

	f := js.GetPath("some_number").MustF64(99.0)
	assert.Equal(t, f, 1.0)

	js.SetPath([]string{}, map[string]interface{}{"foo": "bar"})

	s, err := js.GetPath("foo").S()
	assert.Equal(t, nil, err)
...
func TestPathWillAugmentExisting(t *testing.T) {
	js, err := NewB([]byte(`{"this":{"a":"aa","b":"bb","c":"cc"}}`))
	assert.Equal(t, nil, err)
...
func TestPathWillOverwriteExisting(t *testing.T) {
	...
	js, err := NewB([]byte(`{"this":{"a":0.1,"b":"bb","c":"cc"}}`))
	assert.Equal(t, nil, err)

	js.SetPath([]string{"this", "a", "foo"}, "bar")

	s, err := js.GetPath("this", "a", "foo").S()
	assert.Equal(t, nil, err)
	assert.Equal(t, "bar", s)
}
...
func TestNewFromReader(t *testing.T) {
	...
	assert.Equal(t, js.Get("test").Get("bignum").MustI64(), int64(9223372036854775807))
	assert.Equal(t, js.Get("test").Get("uint64").MustUI64(), uint64(18446744073709551615))
}

func TestSimplejsonGo(t *testing.T) {
	js, err := NewB([]byte(`{
		"test": {
			"array": [1, "2", 3],
			"arraywithsubs": [
